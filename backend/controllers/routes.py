...
def inserir_cliente():
    ...
    d = form_or_json()
    dados = {k: d.get(k) for k in ['nome','cpf','telefone','email','cidade','torce_flamengo','assiste_one_piece','de_sousa'] if k in d}
    for b in ['torce_flamengo','assiste_one_piece','de_sousa']:
        if b in dados: dados[b] = parse_bool(dados[b])
    if img: dados['imagem'] = img
    try:    return ok(mgr_cli.inserir(**dados).to_dict(), 201)
...
@produtos_bp.get('/')
def listar_produtos():
    q=request.args.get('nome'); cat=request.args.get('categoria')
    mari=request.args.get('fabricado_mari'); baixo=request.args.get('estoque_baixo')
...
@carros_bp.get('/')
def listar_carros():
    status=request.args.get('status'); q=request.args.get('nome')
...
def inserir_vendedor():
    ...
    if img: dados['imagem'] = img
    try:    return ok(mgr_ven.inserir(**dados).to_dict(), 201)
    except Exception as e: return err(str(e))
